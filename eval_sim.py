...
def main():
    ...
    parser.add_argument(
        "--dat", default="SpikeEEGBuildEval", type=str, help="data loader"
    )
    parser.add_argument(
        "--test", default="test_sample_source1.mat", type=str, help="test dataset name"
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=float,
        help="ratio between real noise and gaussian noise",
    )
    parser.add_argument(
        "--lfreq",
        default=-1,
        type=int,
        help="filter EEG data to perform narrow-band analysis",
    )
    parser.add_argument(
        "--hfreq",
        default=-1,
    ...
    if os.path.isfile(fn):
        ...
        net = network.__dict__[checkpoint["arch"]](*checkpoint["attribute_list"]).to(
            device
        )  # redefine the weights architecture
        ...
        handler = logging.FileHandler(
            result_root + "/outputs_{}.log".format(checkpoint["arch"])
        )
        handler.setLevel(logging.INFO)
        logger.addHandler(handler)
        logger.info(
            "=================== Evaluation mode: {} ====================================".format(
        ...
        for batch_idx, sample_batch in enumerate(test_loader):
            ...
            for kk in range(out.size(0)):
                eval_dict["all_nmm"].append(
                    nmm[kk, :, label[kk, :, 0]]
                )  # Only save activity in the center region
                # lb = label[kk, :, :]                                    # Save activities in all source regions
                # eval_dict['all_nmm'].append(nmm[kk, :, lb[np.logical_not(ispadding(lb))]])

    # Convert numpy arrays to lists for proper MATLAB cell array conversion
    for key in ["all_regions", "all_out", "all_nmm"]:
        if key in eval_dict and isinstance(eval_dict[key], list):
            eval_dict[key] = eval_dict[key]
        elif key in eval_dict:
            eval_dict[key] = [
                item.tolist() if isinstance(item, np.ndarray) else item
                for item in eval_dict[key]
            ]
