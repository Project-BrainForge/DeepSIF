...
def main():
    ...
    parser.add_argument(
        "--train",
        default="train_sample_source1.mat",
        type=str,
    ...
    parser.add_argument("--model_id", default=2, type=int, help="model id")
    ...
    parser.add_argument(
        "--fwd", default="leadfield_32_20k.mat", type=str, help="forward matrix to use"
    )
    ...
    fwd = loadmat("anatomy/{}".format(args.fwd))["downsampled_fwd"]
    ...
    logger.info(
        "Training data is {}, and testing data is {}".format(args.train, args.test)
    )
    ...
    train_data = loaders.__dict__[args.dat](
        data_root + args.train, fwd=fwd, args_params={"dataset_len": 4}
    )
    ...
    test_data = loaders.__dict__[args.dat](
        data_root + args.test, fwd=fwd, args_params={"dataset_len": 4}
    )
    ...
    if args.resume:
        ...
        if os.path.isfile(fn):
            ...
            checkpoint = torch.load(
                fn, map_location=torch.device("cpu"), weights_only=False
            )
            ...
            ).to(device)  # redefine the weights architecture
            ...
            print(
                "=> Loaded checkpoint epoch {}, current results: {}".format(
                    args.resume, best_result
                )
            )
            ...
            print(
                "WARNING: no checkpoint found at '{}', use random weights".format(
                    args.resume
                )
            )
    ...
    # =============================== TRAINING =======================================================================================================
    print(
        f"Starting training loop. Start epoch: {args.start_epoch + 1}, End epoch: {args.epoch}"
    )
    for epoch in range(args.start_epoch + 1, args.epoch):
        ...
        print_s = (
            "Epoch {}: Time:{:6.2f}, ".format(epoch, time.time() - start_time)
            + "Train Loss:{:06.5f}".format(train_loss[-1])
            + ", Test Loss:{:06.5f}".format(test_loss[-1])
        )
        logger.info(print_s)
        print(print_s)
        ...
        if args.save:
            ...
            savemat(
                result_root + "/train_test_error.mat",
                {"train_loss": train_loss, "test_loss": test_loss},
            )
            savemat(
                result_root + "/train_test_loss_epoch{}.mat".format(epoch),
...
def train(train_loader, model, criterion, optimizer, args_params):
    ...
    for batch_idx, sample_batch in enumerate(train_loader):
        ...
        print(
            f"Input data stats: min={data.min().item():.6f}, max={data.max().item():.6f}, mean={data.mean().item():.6f}"
        )
        print(
            f"Target NMM stats: min={nmm.min().item():.6f}, max={nmm.max().item():.6f}, mean={nmm.mean().item():.6f}"
        )
        ...
        print(
            f"Model output stats: min={out.min().item():.6f}, max={out.max().item():.6f}, mean={out.mean().item():.6f}"
        )
        ...
        if (batch_idx + 1) % 500 == 0:
            print_s = "batch_idx_{}_time_{}_train_loss_{}".format(
                batch_idx, time.time() - start_time, train_loss[-1]
            )
            logger.info(print_s)
