...
def main():
    ...
    for pii in subject_list:
        ...
        if len(flist) == 0:
            ...
            continue
        flist = sorted(
            flist, key=lambda name: int(os.path.basename(name)[4:-4])
        )  # sort file based on nature number
        test_data = []
        ...
        for i in flist:
            ...
            # Get expected number of channels from model attributes
            expected_channels = checkpoint["attribute_list"][0]
            current_channels = data.shape[1]

            # Pad data if necessary (e.g., from 64 channels to 75 channels)
            if current_channels < expected_channels:
                print(
                    f"Padding data from {current_channels} to {expected_channels} channels"
                )
                padding = np.zeros(
                    (data.shape[0], expected_channels - current_channels)
                )
                data = np.hstack((data, padding))
            elif current_channels > expected_channels:
                print(
                    f"Warning: Truncating data from {current_channels} to {expected_channels} channels"
                )
        ...
        savemat(
            folder_name + "/rnn_test_{}_{}.mat".format(args.model_id, fn[-8:]),
            {"all_out": all_out},
        )
        print(
            "Save output as:",
